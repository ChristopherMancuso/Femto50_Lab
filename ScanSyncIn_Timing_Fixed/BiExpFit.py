...
def BiExpFit(data,PMTstart,PMTstop,ShortLT,LongLT,TimeArray,label):
    
    def ExpFun_bi(x,a1):
        return a1*np.exp(-(1/ShortLT)*x) + (1-a1)*np.exp(-(1/LongLT)*x)
    ...
    try:
        ...
        popt, pcov = curve_fit(ExpFun_bi, TimeArray_to_fit, LTcurve_to_fit, p0=[0.5])
        ...
        ax1.annotate('$t_2$ = %.3f'%LongLT, xy=(0.45, 0.75), xycoords='axes fraction', color='k', fontsize=12)
        ...
        ## save the figures ##
        fig1.savefig('results/BiFit_2LTfix_'+label+'_PMTs%.fto%.f'%(PMTstart,PMTstop),dpi=200)
        fig2.savefig('results/BiFit_2LTfix_Residual_'+label+'_PMTs%.fto%.f'%(PMTstart,PMTstop),dpi=200)
